...
task_mat<-read.table('task_matrix.common_names.mfi_raw.tsv',sep='\t',header=TRUE,stringsAsFactors=TRUE,row.names=1)
...
setwd("/home/pramod/Documents/GitHub/gitlab/mcia-model/cmi_pb_datasets/processed/imputed")
...
all_tasks <- c(tasks_ab, tasks_cytof, tasks_seq)
```
...
gs_2020 <- as_tibble(meta.2020["age_at_boost"]) %>%
    rownames_to_column("rowid") %>%
    filter(rowid %in% rownames(gs_2020_temp)) %>%
    column_to_rownames("rowid")
  
colnames(gs_2020) <- "age"
...
gs_2021 <- as.data.frame(meta.2021[,"age_at_boost"])
colnames(gs_2021)<- "age"
rownames(gs_2021) = rownames(meta.2021)
...
write.table(rank_predict_out,'Baseline_Pramod_2020_preds_06212023.tsv',sep='\t')
```